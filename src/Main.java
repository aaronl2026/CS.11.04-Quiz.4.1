public class Main {
    ...
    public static int notDivisibleBy235(int number){
        ...
        for(int i = 0; i < number; i++){
            if(i % 2 != 0 && i % 3 != 0 && i % 5 != 0) count = count + 1;
        }
        return count;
    }
    ...
    public static String camelCaseMe(String str){
        String finalstring = "";
        for(int i = 0; i < str.length(); i++){
            if (i == str.length() - 1 && (str.charAt(i) == '_' || str.charAt(i) == '-')) finalstring = finalstring + "";
            else if(str.charAt(i) == '-' || str.charAt(i) == '_'){
                String camelcase = str.substring(i+1,i+2);
                finalstring = finalstring + camelcase.toUpperCase();
                i++;
            }
            else finalstring = finalstring + str.charAt(i);
        }
        return finalstring;
    }
